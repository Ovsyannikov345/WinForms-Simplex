...
namespace SimplexMethod
{
    ...
    public class Simplex
    {
        ...
        public Tuple<List<SimplexSnap>, SimplexResult, double[]> GetResult()
        {
            ...
            double[] variableValues = GetVariableValues();

            return new Tuple<List<SimplexSnap>, SimplexResult, double[]>(snaps, result.result, variableValues);
        }

        private double[] GetVariableValues()
        {
            double[] values = new double[function.variables.Length]; // Number of original variables

            for (int i = 0; i < C.Length; i++)
            {
                if (C[i] < function.variables.Length)  // Ignore slack/artificial variables
                {
                    values[C[i]] = b[i]; // Assign corresponding b[i] to variable C[i]
                }
            }
